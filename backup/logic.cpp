// ============================================================
//  logic.cpp — Womo Energy Core v5.0
//
//  v5.0 Änderungen:
...
void logic_evaluate() {
    ...
    // ── On-board RGB-LED: Zustand nach Priorität (hoch→niedrig) ──
    // v5.0: keine BMS-Warnflags mehr → Diagnose aus bms_ok/mppt_ok/
    // SoC/Landstrom/Aktorstatus abgeleitet.
    RgbState rs;
    if      (!bms_ok)                      rs = RGB_FAULT_BMS;
    else if (!mppt_ok)                     rs = RGB_FAULT_MPPT;
    else if (soc < RGB_SOC_LOW_PCT)        rs = RGB_SOC_LOW;
    else if (landstrom)                    rs = RGB_LANDSTROM;
    else if (g_io.relayDPlus || g_io.mosfetGel || g_io.wrRemote)
                                           rs = RGB_ACTIVE;
    else                                   rs = RGB_IDLE_OK;
    g_rgbState = rs;
}
